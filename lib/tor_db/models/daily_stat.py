...
class DailyStat(db.Entity):
    ...
    def get_stats(klass):
        ...
        r[
            "unique_visitors"
        ] = tor_db.models.request_log.RequestLog.unique_visitors_since(event_horizon)
        r["total_onions_all"] = count(
            d for d in tor_db.models.domain.Domain if d.last_alive > event_horizon
        )
        r["up_right_now_all"] = count(
            d
            for d in tor_db.models.domain.Domain
            if d.last_alive > event_horizon and d.is_up == True
        )
        r["new_onions_all"] = count(
            d
            for d in tor_db.models.domain.Domain
            if d.last_alive > event_horizon and d.created_at > event_horizon
        )
        r["new_onions"] = count(
            d
            for d in tor_db.models.domain.Domain
            if d.last_alive > event_horizon
            and d.created_at > event_horizon
            and d.is_subdomain == False
            and d.is_crap == False
            and d.is_banned == False
        )
        r["total_onions"] = count(
            d
            for d in tor_db.models.domain.Domain
            if d.last_alive > event_horizon
            and d.is_subdomain == False
        ...
        r["banned"] = count(
            d for d in tor_db.models.domain.Domain if d.is_banned == True
        )
        r["banned_up_last_24"] = count(
            d
            for d in tor_db.models.domain.Domain
