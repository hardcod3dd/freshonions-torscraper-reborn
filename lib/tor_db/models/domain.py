...
class Domain(db.Entity):
    ...
    description_json = Optional(str, 10240)
    ...
    def random(klass, number=1000):
        # v3 onion addresses are 56 base32 (a-z2-7) characters
        chars = "abcdefghijklmnopqrstuvwxyz234567"
        onions = []
        for i in range(number):
            r = "".join(random.choice(chars) for _ in range(56))
    ...
    def is_onion_url(klass, url):
        ...
        try:
            parsed_url = urllib.parse.urlparse(url)
            host = parsed_url.hostname
            # v3 onion: 56 base32 lowercase chars (a-z2-7), with optional subdomain labels
            if re.match(r"^([a-z0-9-]{1,63}\.)*[a-z2-7]{56}\.onion$", host):
                return True
            else:
                return False
        except:
