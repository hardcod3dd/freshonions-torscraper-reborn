...
# Set SITE_ONION to the onion hostname of your own freshonions instance to
# prevent the crawler from indexing itself (e.g. "yourv3address56chars.onion").
SITE_ONION = os.environ.get("SITE_ONION", "")
...
class TorSpider(scrapy.Spider):
    ...
    spider_exclude = []  # add v3 onion hostnames to exclude from crawling
    ...
    def update_page_info(self, url, title, code, is_frontpage=False, size=0):
        ...
        if SITE_ONION and host == SITE_ONION:
            return False
    ...
    def parse(self, response, recent_alive_check=False):
        ...
        host = parsed_url.hostname
        if not SITE_ONION or host != SITE_ONION:
